...
```
$ git clone git@github.com:axmaxon/gr-kata.git
```
...
В корне проекта находится файл `demo.rb`, который демонстрирует работу системы в части работы метода
GildedRose#update_quality. Данное мини-приложение создает образец списка элементов, в нём также имитируется течение времени 
(2 дня - по умолчанию, или количество дней указанных пользователем), "в конце каждого дня" с помощью GildedRose#update_quality метода,
обновляются свойства товаров.
Далее - в консоли пользователю предоставляется информация об изменении свойств товара для кажого дня.
